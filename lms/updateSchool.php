  ...
  <?php 
      ...
      if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $query = "UPDATE schools SET `school Name`='$schoolName', `school Level`='$schoolLevel', `Phone`='$phone', `Email`='$email', `board name`='$boardName', `school number`='$schoolNumber',
         `schoolSpecialConditions` = '$schoolSpecialConditions', `street`='$street', `city`='$city', `province`='$province', `postalCode`='$postalCode', `phone`='$phone', `fax`='$fax', `dateOpen`= '$dateOpen'  
         `schoolGrade`='$schoolGrade', `schoolLanguage`='$schoolLanguage' WHERE id=$id";
        $result = mysqli_query($connect, $query);
        ...
        <div class="col-md-6">
          <form action="updateSchool.php" method="POST">
            ...
            <div class="mb-3">
              ...
              <input type="text" class="form-control" id="boardName" name="boardName"  value="<?php echo $school['BoardName'] ?>">
            </div>
